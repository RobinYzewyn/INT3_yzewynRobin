...
function caleandar(el, data, settings) {
  ...
  let events = [
    //Februari : 1 Maart: 2 April: 3 Mei: 4 Juni: 5 Juli: 6 Aug: 7 Sep: 8 Okt: 9 Nov: 10 Dec: 11 Jan: 12
    //{ Date: new Date(2020, 10, 20), Title: "", Link: "", },
  ];
  const init = () =>{
    let currentPage = (window.location.href).split('?')[1];
    if(currentPage == "page=profiel" || currentPage == "page=opdrachten" || currentPage == "page=opdrachttoevoegen" || currentPage == "page=vaktoevoegen" || currentPage == "page=schoolagenda" || currentPage == "page=schoolagendaToevoegen"){
  ...
  importTask = () =>{
    ...
    for (let i = 0; i < tasks.length; i++) {
      let date = (tasks[i].children[1].children[2].textContent).substr(0,10);
      let year = (date.substr(0,4));
      let month = parseInt((date.substr(5,7)).slice(0,2));
      montje = correctMonth(month);
      let day = (date.substr(8,12));
      events[i] = { Date: new Date(year, montje, day), Title: "", Link: "", }
    }
  }
  init();
}
